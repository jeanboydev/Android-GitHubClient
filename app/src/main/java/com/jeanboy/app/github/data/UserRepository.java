...
//    public LiveData<User> getRemoteUser(int userId) {
...
//
//    public LiveData<User> getCacheUser(int userId) {
...
//        if (load != null) {//数据库有缓存
//            userCache.put(String.valueOf(userId), load);//存入内存缓存
//            return load;
//        }
//
//        //没有缓存
//        final MutableLiveData<User> data = new MutableLiveData<>();
//        userCache.put(String.valueOf(userId), data);//创建LiveData放入缓存
//        //通过服务器获取
//        webApi.getUser(userId).enqueue(new Callback<User>() {
//            @Override
//            public void onResponse(Call<User> call, Response<User> response) {
